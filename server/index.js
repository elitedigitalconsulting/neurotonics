...
app.post('/create-checkout-session', async (req, res) => {
  ...
  // --- Validate cart and shipping against server-side price truth ---
  const pricing = buildCheckoutPricing({ items, shipping, shippingAddress });
  ...
  // Add authoritative server-calculated shipping as a separate line item.
  if (pricing.shippingFeeCents > 0) {
    lineItems.push({
      price_data: {
        currency: 'aud',
        product_data: {
          name: `Delivery — ${pricing.shipping.name || pricing.shipping.zone || 'Standard'}`,
          ...(pricing.shipping.estimatedDays && { description: pricing.shipping.estimatedDays }),
        },
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: {
        ...
        shippingFee: String(pricing.shippingFeeCents),
        shippingZone: pricing.shipping.zone || 'none',
        shippingOption: pricing.shipping.name || 'none',
